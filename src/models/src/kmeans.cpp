...
void LoadCluster(const int start, const int end, const int N, const int dim, cl::Buffer index, cl::Buffer points, std::vector<Matrix<double, Dynamic, Dynamic>>& clusters){
...
std::vector<Matrix<double, Dynamic, Dynamic>> KMeans(int k, DataTable& input){
    if(input.Size() * k > Core_Bond::MAX_BUFFER_SIZE) throw new std::exception("Insufficient Buffer Space!");
    ...
    std::vector<Matrix<double, Dynamic, Dynamic>> clusters;

    // Initialize Clusters & Centroids
    for (int i = 0; i < k; i++){
        centroids.row(i) = input.GetRow(i);
        clusters.push_back(Matrix<double, Dynamic, Dynamic>());
        clusters[i].setZero(maxEntries, dimensions);
    }
    ...
    double maxThreads = std::thread::hardware_concurrency();
    int step = std::ceil(maxEntries / maxThreads);
